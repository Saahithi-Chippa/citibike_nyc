...
def download_taxi_zone_lookup_csv(data_dir, url="https://d37ci6vzurychx.cloudfront.net/misc/taxi_zone_lookup.csv", log=True):
    """
    Downloads the taxi_zone_lookup.csv file if it does not exist.

    Parameters:
        data_dir (str or Path): Directory where the data will be stored.
        url (str): URL of the taxi_zone_lookup.csv file.
        log (bool): Whether to log progress messages.
    ...
    if not csv_path.exists():
        if log:
            print(f"Downloading taxi_zone_lookup.csv from {url}...")
        try:
            response = requests.get(url, timeout=10)
            response.raise_for_status()  # Raise an HTTPError for bad responses
            with open(csv_path, "wb") as f:
                f.write(response.content)
            if log:
                print(f"File downloaded and saved to {csv_path}")
        except requests.exceptions.RequestException as e:
            raise Exception(f"Failed to download file from {url}: {e}")
    else:
        if log:
