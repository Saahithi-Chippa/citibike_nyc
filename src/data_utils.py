...
def fetch_raw_data(year: int, month: int) -> Path:
    ...
    path = RAW_DATA_DIR / f"rides_{year}_{month:02}.csv"
...
def filter_nyc_bike_data(rides: pd.DataFrame, year: int, month: int) -> pd.DataFrame:
    """
    Filters NYC Citibike ride data for a specific year and month, removing outliers and invalid records.

    Args:
        rides (pd.DataFrame): DataFrame containing NYC Citi Bike ride data.
    ...
    rides["end_station_id"] = (rides["end_station_id"]).astype(str)

    rides["start_station_id"] = rides["start_station_id"].str.strip().str.replace("_", '')

    # Handle mixed data types in start_station_id column
    rides["start_station_id"] = pd.to_numeric(rides["start_station_id"], errors='coerce')
    # Drop Missing start station values
    rides.dropna(subset=["start_station_id"], inplace = True)
    ...
    # Combine all filters
    final_filter = (
        duration_filter & date_range_filter
    )
...
def load_and_process_bike_data(
    ...
    """
    Load and process NYC Citi Bike ride data for a specified year and list of months.
    ...
    # combined_rides.to_csv(PROCESSED_DATA_DIR/f"rides_processed_{year}.csv", index=False)
...
def transform_raw_data_into_ts_data(rides: pd.DataFrame) -> pd.DataFrame:
    ...
    rides["start_hour"] = rides["start_datetime"].dt.floor("h")

    # Aggregate and fill gaps
    agg_rides = (
        rides.groupby(["start_hour", "start_station_id"])
        .size()
    ...
    agg_rides_all_slots = (
        fill_missing_rides_full_range(
            agg_rides, "start_hour", "start_station_id", "rides"
        )
        .sort_values(["start_station_id", "start_hour"])
        .reset_index(drop=True)
    )

    # important
    agg_rides_all_slots = agg_rides_all_slots.astype(
        {"start_station_id": "float64", "rides": "int16"}
    )
...
def transform_ts_data_info_features_and_target_loop(
    ...
    Parameters:
        df (pd.DataFrame): The input DataFrame containing time series data with 'start_hour' column.
    ...
    Returns:
        tuple: (features DataFrame with start_hour, targets Series, complete DataFrame)
    """
    # Get all unique location IDs
    ...
    for location_id in location_ids:
        try:
            ...
            location_data = df[df["start_station_id"] == location_id].reset_index(
                drop=True
            )
    ...
    final_df["start_hour"] = pd.to_datetime(final_df["start_hour"])

    # Extract features (including start_hour), targets, and keep the complete DataFrame
...
def transform_ts_data_info_features_and_target(
    ...
    features = final_df[feature_columns + ["start_hour", "start_station_id"]]
    # features["start_hour"] = pd.to_datetime(features["start_hour"])
    targets = final_df["target"]
