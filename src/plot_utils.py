...
def plot_aggregated_time_series(
    ...
    """
    Plots the time series data for a specific location from NYC Citi Bike data.
    ...
    location_features = features.iloc[row_id].copy()
    ...
    location_features["start_hour"] = pd.to_datetime(location_features["start_hour"])

    # Generate corresponding timestamps for the time series
    time_series_dates = pd.date_range(
        start=location_features["start_hour"]
        - timedelta(hours=len(time_series_columns)),
        end=location_features["start_hour"],
        freq="h",
    )

    # Create the plot title with relevant metadata
    title = f"Start Hour: {location_features['start_hour']}, Location ID: {location_features['start_station_id']}"
    ...
    if predictions is not None:
        fig.add_scatter(
            x=time_series_dates[-1:],  # Last timestamp
            y=predictions.to_numpy(), # Predicted value
            line_color="red",
            mode="markers",
            marker_symbol="x",
            marker_size=15,
...
def plot_prediction(features: pd.DataFrame, prediction: int):
    ...
    # Create the plot title with relevant metadata
    title = f"Start Hour: {pickup_hour}, Station ID: {features['Start_station_id'].iloc[0]}"
